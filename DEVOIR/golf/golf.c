...
int creerParcours( typeParcours *tab[], int nbParcours )
{
	...
	// creation du parcours � l'indice nbParcours
...
void afficherInfoTrou(typeTrou unTrou){
    ...
    printf("trou numéro %d | distance %.2f mètres | par %d | %d coups maxi",unTrou.numero,unTrou.distance,unTrou.par,unTrou.nbMaxCoups);
...
/*****************************************************************************************/
	...
	{
		afficherInfoTrou( p->listeTrous[i] );
		sc2->points[i] = jouerTrou( p->listeTrous[i], niveau );
	}
