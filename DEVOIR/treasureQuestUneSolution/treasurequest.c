...
int calculDegats(typePersonnage *attaquant, typePersonnage*defenseur) {
    int defense, attaque, degats;
    attaque = calculAttaque(attaquant);
    defense = calculDefense(defenseur);
...
void mettreAJourNiveau(typePersonnage *perso, unsigned int exp) {
    ...
    while (perso->experience >= EXPERIENCEPASSAGE) {
        perso->niveau++;
...
void faireUnDuel(typePersonnage *j1, typePersonnage *j2) {
    ...
    if (j1gagne) {
        ...
        printf("experience joueur : %d\n", exp);
        mettreAJourNiveau(j1, exp);
    }
...
void sauvegarde(typePersonnage *joueur, int x, int y, char tab[LARGEUR][HAUTEUR], int etage) {
    FILE *fic;
    int retour;
    // ouverture du fichier en ecriture/creation
    fic = fopen(FICHIERSAUVEGARDE, "w+");
    if (fic == NULL) {
        printf("%s\n", strerror(errno));
        // exit(errno);
    } else {
        retour = fwrite(&etage, sizeof (int), 1, fic);
        if (retour != 1) {
            printf("erreur d'ecriture avec fwrite %s\n", strerror(errno));
            exit(retour);
        }
        retour = fwrite(&x, sizeof (int), 1, fic);
        if (retour != 1) {
            printf("erreur d'ecriture avec fwrite %s\n", strerror(errno));
            exit(retour);
        }
        retour = fwrite(&y, sizeof (int), 1, fic);
        if (retour != 1) {
            printf("erreur d'ecriture avec fwrite %s\n", strerror(errno));
            exit(retour);
        }
        retour = fwrite(joueur, sizeof (typePersonnage), 1, fic);
        if (retour != 1) {
            printf("erreur d'ecriture avec fwrite %s\n", strerror(errno));
            exit(retour);
        }
        retour = fwrite(tab, LARGEUR*HAUTEUR, 1, fic);
        if (retour != 1) {
            printf("erreur d'ecriture avec fwrite %s\n", strerror(errno));
            exit(retour);
        }
...
int chargement(typePersonnage *joueur, char tab[LARGEUR][HAUTEUR], int *x, int *y) {
    int etage = -1;
    FILE *fic;
    int retour;
    // ouverture du fichier en ecriture/creation
    fic = fopen(FICHIERSAUVEGARDE, "r");
    if (fic == NULL) {
        printf("%s\n", strerror(errno));
        //      exit(errno);
    } else {
        retour = fread(&etage, sizeof (int), 1, fic);
        if (retour != 1) {
            printf("erreur d'ecriture avec fwrite %s\n", strerror(errno));
            exit(retour);
        }
        retour = fread(x, sizeof (int), 1, fic);
        if (retour != 1) {
            printf("erreur d'ecriture avec fwrite %s\n", strerror(errno));
            exit(retour);
        }
        retour = fread(y, sizeof (int), 1, fic);
        if (retour != 1) {
            printf("erreur d'ecriture avec fwrite %s\n", strerror(errno));
            exit(retour);
        }
        retour = fread(joueur, sizeof (typePersonnage), 1, fic);
        if (retour != 1) {
            printf("erreur d'ecriture avec fwrite %s\n", strerror(errno));
            exit(retour);
        }
        retour = fread(tab, LARGEUR*HAUTEUR, 1, fic);
        if (retour != 1) {
            printf("erreur d'ecriture avec fwrite %s\n", strerror(errno));
            exit(retour);
        }
